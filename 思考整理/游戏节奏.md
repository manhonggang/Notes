...
MMO的装等公式千变万化，但基本来说都是 Jensen 不等式的变种，这里以魔兽世界为例，魔兽世界的装备等级公式：
...
最后的2/3方是为了让整个得出的值偏线性化，便于玩家理解，而前面的1.5次方，实际上是 Jensen 不等式，我们可以设想当一件装备只有一个属性的时候，另外一个装备有两个属性，那么该如何计算他们两个的价值才能平衡？

  

回顾一下 Jensen 不等式：如果f是凸函数，X是随机变量，那么：E[f(X)]>=f(E[X])
